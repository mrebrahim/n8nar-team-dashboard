...
export default function EmployeeDashboard({ user, onLogout }) {
  ...
  async function loadAll() {
    ...
    if (todayRep) {
      ...
      const doneMap = {}
      const valMap = {}
      completions?.forEach(c => { doneMap[c.task_id] = c.is_done; valMap[c.task_id] = c.actual_value })
      setDone(doneMap)
      setKpiValues(valMap)
      const { data: kpis } = await supabase.from('kpi_entries').select('*').eq('report_id', todayRep.id)
      const km = {}
      kpis?.forEach(k => km[k.metric_key] = k.actual_value)
      setKpiValues(prev => ({ ...prev, ...km }))
  ...
  async function submitReport() {
    ...
    for (const task of tasks) {
      await supabase.from('task_completions').upsert({
        report_id: r.id, task_id: task.id,
        is_done: !!done[task.id], actual_value: kpiValues[task.id] || null
  ...
  const reachPct = reachTarget ? Math.min(Math.round((reachTarget.current_value / reachTarget.target_value) * 100), 100) : 0
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button onClick={onLogout} style={{
            ...
            color: 'var(--text3)', borderRadius: 6, padding: '5px 12px', fontSize: 11, cursor: 'pointer'
          }}>خروج</button>
        </div>
      </div>
      ...
      <div style={{ background: 'var(--bg1)', borderBottom: '0.5px solid var(--border)', padding: '0 20px', display: 'flex', gap: 4 }}>
        {TABS.map(t => (
          <button key={t.id} className={`nav-btn ${tab === t.id ? 'active' : ''}`} onClick={() => setTab(t.id)}
            style={{ padding: '12px 14px', borderRadius: '8px 8px 0 0', borderBottom: tab === t.id ? `2px solid ${color}` : '2px solid transparent', color: tab === t.id ? color : 'var(--text3)' }}>
      ...
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '20px 16px' }}>
        ...
        {tab === 'targets' && (
          <div className="fade-in">
            {/* My completion today */}
            <div className="card" style={{ marginBottom: 16, borderColor: color + '40' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
                ...
                <div style={{ textAlign: 'left' }}>
                  <div style={{ fontSize: 28, fontWeight: 800, color: completionPct >= 100 ? 'var(--green)' : completionPct >= 70 ? 'var(--amber)' : color }}>
                    {completionPct}%
                  </div>
              ...
              <div className="prog-bar" style={{ height: 8 }}>
                <div className="prog-fill" style={{
                  width: completionPct + '%',
                  background: completionPct >= 100 ? 'var(--green)' : completionPct >= 70 ? 'var(--amber)' : color
                }} />
              </div>
              {completionPct >= 100 && (
            ...
            <div className="grid2" style={{ marginBottom: 16 }}>
              <div className="stat-card" style={{ borderColor: 'rgba(46,204,113,.2)' }}>
                <div className="stat-lbl">المبيعات</div>
                <div className="stat-val" style={{ color: 'var(--green)' }}>
                  {salesTarget?.current_value?.toLocaleString() || 0}
                  <span style={{ fontSize: 13, color: 'var(--text3)', fontWeight: 400 }}> / {TEAM_GOAL_SALES.toLocaleString()}</span>
                </div>
                <div className="prog-bar">
                  <div className="prog-fill" style={{ width: salesPct + '%', background: 'var(--green)' }} />
                </div>
                <div className="stat-sub">{salesPct}% من الهدف</div>
              </div>
              <div className="stat-card" style={{ borderColor: 'rgba(74,144,217,.2)' }}>
                <div className="stat-lbl">الـ Reach الكلي</div>
                <div className="stat-val" style={{ color: color }}>
                  {reachTarget?.current_value?.toLocaleString() || 0}
                  <span style={{ fontSize: 13, color: 'var(--text3)', fontWeight: 400 }}> / {(TEAM_GOAL_REACH / 1000000).toFixed(0)}M</span>
                </div>
                <div className="prog-bar">
                  <div className="prog-fill" style={{ width: reachPct + '%', background: color }} />
                </div>
                <div className="stat-sub">{reachPct}% من الـ 10 مليون</div>
              </div>
            </div>

            {/* My KPI Targets */}
            <div style={{ fontSize: 11, color: 'var(--text3)', marginBottom: 10, fontWeight: 600, letterSpacing: 1 }}>تارجتي اليومي</div>
            <div className="card">
              {tasks.map(t => (
                <div key={t.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '0.5px solid var(--border)' }}>
                  <div style={{ fontSize: 13, color: done[t.id] ? 'var(--text3)' : 'var(--text1)', textDecoration: done[t.id] ? 'line-through' : 'none', flex: 1 }}>{t.task_text}</div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0, marginRight: 12 }}>
                    {t.kpi_target && <span style={{ fontSize: 11, color: color, fontFamily: 'var(--mono)', fontWeight: 600 }}>{t.kpi_target} {t.kpi_unit}</span>}
            ...
            {historyReports.length > 0 && (
              <>
                <div style={{ fontSize: 11, color: 'var(--text3)', margin: '16px 0 10px', fontWeight: 600, letterSpacing: 1 }}>آخر 7 أيام</div>
                <div style={{ display: 'flex', gap: 8 }}>
                  {historyReports.map(r => (
                    <div key={r.id} style={{ background: 'var(--bg2)', border: '0.5px solid var(--border)', borderRadius: 8, padding: '8px 12px', textAlign: 'center', minWidth: 60 }}>
                      <div style={{ fontSize: 10, color: 'var(--text3)', marginBottom: 4 }}>
                        {new Date(r.report_date).toLocaleDateString('ar-EG', { weekday: 'short' })}
                      </div>
                      <div style={{ fontSize: 14, color: 'var(--green)', fontWeight: 700 }}>✓</div>
                    </div>
                  ))}
                </div>
              </>
          ...
          <div className="fade-in">
            ...
            <div className="card">
              {tasks.map((t, i) => {
                ...
                return (
                  <div key={t.id} className="fade-in" style={{ animationDelay: `${i * 0.05}s`, display: 'flex', alignItems: 'flex-start', gap: 12, padding: '12px 0', borderBottom: i < tasks.length - 1 ? '0.5px solid var(--border)' : 'none' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      {t.kpi_target && (
                        <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 4, display: 'flex', alignItems: 'center', gap: 6 }}>
                          <span style={{ color: color }}>هدف:</span> {t.kpi_target} {t.kpi_unit}
                          {isDone && kpiValues[t.id] && (
                            <span style={{ color: 'var(--green)' }}>— حققت: {kpiValues[t.id]} {t.kpi_unit}</span>
                          )}
                        </div>
                      )}
                    </div>
                    {isDone && (
                      <span className="tag badge-ok" style={{ flexShrink: 0, marginTop: 2 }}>✓</span>
                    )}
                  </div>
                )
              })}
        ...
        {tab === 'report' && (
          <div className="fade-in">
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: report ? 'var(--green)' : 'var(--text3)', marginTop: 2 }}>
                  {report ? '✓ تم رفع تقرير اليوم' : 'لم يُرفع بعد'}
                </div>
              </div>
            </div>
                  ...
                  {t.kpi_target && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, paddingRight: 30 }}>
                      ...
                      <input type="number" value={kpiValues[t.id] || ''} onChange={e => setKpiValues(p => ({ ...p, [t.id]: e.target.value }))}
                        placeholder={`هدف: ${t.kpi_target} ${t.kpi_unit}`}
            ...
            {saved && (
              <div style={{ background: 'rgba(46,204,113,.1)', border: '0.5px solid rgba(46,204,113,.3)', borderRadius: 10, padding: '12px 16px', marginBottom: 14, color: 'var(--green)', fontSize: 13, textAlign: 'center' }}>
            ...
            <button className="primary-btn" onClick={submitReport} disabled={saving} style={{ background: color }}>
              {saving
                ? <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8, justifyContent: 'center' }}><span style={{ width: 14, height: 14, border: '2px solid rgba(255,255,255,.3)', borderTopColor: '#fff', borderRadius: '50%', animation: 'spin 1s linear infinite', display: 'inline-block' }} />جاري الحفظ...</span>
                : report ? 'تحديث التقرير' : 'إرسال التقرير'
              }
            </button>
          </div>
        )}
